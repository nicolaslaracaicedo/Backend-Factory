...
import fs from 'fs';
import type { LiquidacionCompraConDetalles } from '../models/liquidaciones_compra.model';
...
export async function generarPdfLiquidacionCompra(
  ...
  for (const d of lc.detalles) {
    ...
    else { subIva += Number(d.subtotal); ivaMainPct = Number(d.porcentaje_iva) || 15; }
  }

  const subtotalSinImp = Number(lc.subtotal_sin_impuesto);
  const descTotal      = Number(lc.descuento_total);
  const total          = Number(lc.total);
  const isAutorizado   = lc.estado === 'AUTORIZADO';

  const barcodeBuffer = lc.clave_acceso ? await makeBarcodeBuffer(lc.clave_acceso) : null;

  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({
      size: 'A4', margin: 0,
      info: { Title: `Liquidación de Compra ${lc.numero_comprobante ?? ''}`, Author: empresa.razon_social ?? '' },
    });
    ...
    const drawFooter = () => {
      ...
      if (lc.clave_acceso) {
        doc.fillColor(MUTED).font('Helvetica').fontSize(5.5)
           .text(`Clave de acceso: ${lc.clave_acceso}`, M, fy + 6, { width: CW, align: 'center', lineBreak: false });
      }
    ...
    doc.font('Helvetica-Bold').fontSize(7.5).fillColor(TEXT_ON_S)
       .text('LIQ. DE COMPRA', BX_PAD, by + 5, { width: BADGE_IW, align: 'center', lineBreak: false });
    by += 22;
    ...
    by += 8;
    doc.font('Helvetica-Bold').fontSize(7.5).fillColor(TEXT_ON_P).text(lc.numero_comprobante ?? '-', BX_PAD, by, { width: BADGE_IW, align: 'center', lineBreak: false });
    by += 11;

    doc.moveTo(BX_PAD, by).lineTo(BX_PAD + BADGE_IW, by).strokeColor(lighten(PRIMARY, 0.3)).lineWidth(0.4).stroke();
    by += 5;

    if (isAutorizado && lc.fecha_autorizacion) {
      ...
      by += 8;
      doc.font('Helvetica-Bold').fontSize(6.5).fillColor(TEXT_ON_P).text(fmtDateTime(lc.fecha_autorizacion), BX_PAD, by, { width: BADGE_IW, align: 'center', lineBreak: false });
      by += 10;
    }

    const ambLabel = (lc as any).id_ambiente === 2 ? 'PRODUCCIÓN' : 'PRUEBAS';
    ...
    if (lc.clave_acceso) {
      ...
      doc.font('Helvetica').fontSize(5.8).fillColor(MUTED).text(lc.clave_acceso, M + 8, y + 16 + BC_H + 2, { width: CW - 80, lineBreak: false });

      const ESTADO_COLOR: Record<string, string> = {
        AUTORIZADO: '#059669', BORRADOR: '#d97706', RECHAZADA: '#dc2626', ENVIADO: '#0284c7', ANULADA: '#64748b',
      };
      const ec = ESTADO_COLOR[lc.estado] ?? '#64748b';
      doc.rect(M + CW - 70, y + 16 + BC_H - 1, 62, 14).fill(ec);
      doc.font('Helvetica-Bold').fontSize(7).fillColor('#ffffff')
         .text(lc.estado, M + CW - 70, y + 16 + BC_H + 3, { width: 62, align: 'center', lineBreak: false });
    ...
    const provRows: Array<[string, string, string, string]> = [
      ['Razón Social:', lc.razon_social_prov ?? '-', 'Fecha Emisión:', fmtDate(lc.fecha_emision)],
      ['Identificación:', lc.identificacion_prov ?? '-', 'Tipo ID:', lc.tipo_identificacion_prov ?? '-'],
      ['Dirección:', lc.direccion_prov ?? '-', '', ''],
    ];

    const CL_H = 14 + provRows.length * 18 + 4;
    box(M, y, CW, CL_H, GRAY_BG);
    doc.rect(M, y, CW, 14).fill(lighten(PRIMARY, 0.85));
    doc.font('Helvetica-Bold').fontSize(7).fillColor(PRIMARY).text('DATOS DEL PROVEEDOR', M + 8, y + 4, { lineBreak: false });
    ...
    for (const d of lc.detalles) {
      ...
      doc.font('Helvetica-Bold').text(fmt(d.total), last.x+3, ty, { width: last.w-6, align:'right', lineBreak:false });
    ...
    doc.font('Helvetica-Bold').fontSize(7).fillColor(PRIMARY).text('RESUMEN DE IMPUESTOS', TOT_X + 8, y + 4, { lineBreak: false });
