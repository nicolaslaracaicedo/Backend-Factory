...
export async function generarPdfNotaDebito(
  ...
  const ivaTotal   = Number(nd.iva_total);
  const total      = Number(nd.total);
  const isAutorizado = nd.estado === 'AUTORIZADO';

  const barcodeBuffer = nd.clave_acceso ? await makeBarcodeBuffer(nd.clave_acceso) : null;
  ...
  return new Promise((resolve, reject) => {
    ...
    const drawFooter = () => {
      ...
      if (nd.clave_acceso) {
        doc.fillColor(MUTED).font('Helvetica').fontSize(5.5)
           .text(`Clave de acceso: ${nd.clave_acceso}`, M, fy + 6, { width: CW, align: 'center', lineBreak: false });
      }
    ...
    doc.font('Helvetica-Bold').fontSize(8.5).fillColor(TEXT_ON_S)
       .text('NOTA DE DÉBITO', BX_PAD, by + 5, { width: BADGE_IW, align: 'center', lineBreak: false });
    by += 22;
    ...
    by += 8;
    doc.font('Helvetica-Bold').fontSize(7.5).fillColor(TEXT_ON_P).text(nd.numero_comprobante ?? '-', BX_PAD, by, { width: BADGE_IW, align: 'center', lineBreak: false });
    by += 11;

    doc.moveTo(BX_PAD, by).lineTo(BX_PAD + BADGE_IW, by).strokeColor(lighten(PRIMARY, 0.3)).lineWidth(0.4).stroke();
    by += 5;

    if (isAutorizado && nd.fecha_autorizacion) {
      ...
      by += 8;
      doc.font('Helvetica-Bold').fontSize(6.5).fillColor(TEXT_ON_P).text(fmtDateTime(nd.fecha_autorizacion), BX_PAD, by, { width: BADGE_IW, align: 'center', lineBreak: false });
      by += 10;
    }

    const ambLabel = nd.id_ambiente === 2 ? 'PRODUCCIÓN' : 'PRUEBAS';
    ...
    if (nd.clave_acceso) {
      ...
      doc.font('Helvetica').fontSize(5.8).fillColor(MUTED).text(nd.clave_acceso, M + 8, y + 16 + BC_H + 2, { width: CW - 80, lineBreak: false });

      const ESTADO_COLOR: Record<string, string> = {
        AUTORIZADO: '#059669', BORRADOR: '#d97706', RECHAZADA: '#dc2626', ENVIADO: '#0284c7', ANULADA: '#64748b',
      };
      const ec = ESTADO_COLOR[nd.estado] ?? '#64748b';
      doc.rect(M + CW - 70, y + 16 + BC_H - 1, 62, 14).fill(ec);
      doc.font('Helvetica-Bold').fontSize(7).fillColor('#ffffff')
         .text(nd.estado, M + CW - 70, y + 16 + BC_H + 3, { width: 62, align: 'center', lineBreak: false });

      y += SECTION_H + 5;
    }

    // ── DATOS DEL CLIENTE Y REFERENCIA ──────────────────────────────────────
    const halfCW = CW / 2 - 1;
    const compradorRows: Array<[string, string, string, string]> = [
      ['Razón Social:', nd.cli_razon_social ?? '-', 'Fecha Emisión:', fmtDate(nd.fecha_emision)],
      ['RUC / CI:', nd.cli_identificacion ?? '-', 'Doc. Referencia:', nd.factura_ref_numero ?? '-'],
      ['Motivo:', nd.motivo ?? '-', 'Fecha Doc. Ref.:', fmtDate(nd.factura_ref_fecha)],
    ];

    const CL_H = 14 + compradorRows.length * 18 + 4;
    box(M, y, CW, CL_H, GRAY_BG);
    doc.rect(M, y, CW, 14).fill(lighten(PRIMARY, 0.85));
    doc.font('Helvetica-Bold').fontSize(7).fillColor(PRIMARY).text('DATOS DEL CLIENTE', M + 8, y + 4, { lineBreak: false });
    ...
    // ── TABLA DE RAZONES / VALORES ───────────────────────────────────────────
    const TH_H = 20;
    const COL_RAZON_W = CW - 140;
    const COL_VALOR_W = 140;
    const COL_RAZON_X = M;
    const COL_VALOR_X = M + COL_RAZON_W;
    ...
    const drawTH = (atY: number): number => {
      ...
      doc.text('Razón / Concepto', COL_RAZON_X + 6, atY + 6, { width: COL_RAZON_W - 12, lineBreak: false });
      doc.text('Valor', COL_VALOR_X + 6, atY + 6, { width: COL_VALOR_W - 12, align: 'right', lineBreak: false });
      doc.moveTo(COL_VALOR_X, atY + 3).lineTo(COL_VALOR_X, atY + TH_H - 3).strokeColor(lighten(TEXT_ON_S, 0.4)).lineWidth(0.4).stroke();
      return atY + TH_H;
    };
    ...
    for (const d of nd.detalles) {
      ...
      vline(COL_VALOR_X, y + 2, y + rowH - 2, GRAY_LINE, 0.3);

      doc.fillColor(DARK).font('Helvetica').fontSize(8)
         .text(d.razon, COL_RAZON_X + 6, y + 6, { width: COL_RAZON_W - 12, lineBreak: false });
      doc.font('Helvetica-Bold').fontSize(8)
         .text(`$${fmt(d.valor)}`, COL_VALOR_X + 6, y + 6, { width: COL_VALOR_W - 12, align: 'right', lineBreak: false });
    ...
    for (const row of totRows) {
      ...
      doc.font('Helvetica').fontSize(7).fillColor(MUTED).text(row.label, TOT_X + 8, ty2 + 4, { width: LW, lineBreak: false });
      doc.font('Helvetica').fontSize(7).fillColor(DARK).text(`$${fmt(row.value)}`, TOT_X + 8 + LW, ty2 + 4, { width: VW, align: 'right', lineBreak: false });
      ty2 += TOT_ROW_H;
    }
    ...
    doc.font('Helvetica-Bold').fontSize(10).fillColor(TEXT_ON_S).text('VALOR TOTAL:', TOT_X + 8, ty2 + 5, { width: LW, lineBreak: false });
