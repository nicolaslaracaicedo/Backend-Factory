...
export async function generarPdfNotaCredito(
  ...
  const subtotalSinImp = Number(nc.subtotal_sin_impuesto);
  const descTotal      = Number(nc.descuento_total);
  const total          = Number(nc.total);
  const isAutorizado   = nc.estado === 'AUTORIZADO';

  const barcodeBuffer = nc.clave_acceso ? await makeBarcodeBuffer(nc.clave_acceso) : null;
  ...
  return new Promise((resolve, reject) => {
    ...
    const drawFooter = () => {
      ...
      if (nc.clave_acceso) {
        doc.fillColor(MUTED).font('Helvetica').fontSize(5.5)
           .text(`Clave de acceso: ${nc.clave_acceso}`, M, fy + 6, { width: CW, align: 'center', lineBreak: false });
      }
    ...
    doc.font('Helvetica-Bold').fontSize(8.5).fillColor(TEXT_ON_S)
       .text('NOTA DE CRÉDITO', BX_PAD, by + 5, { width: BADGE_IW, align: 'center', lineBreak: false });
    by += 22;
    ...
    by += 8;
    doc.font('Helvetica-Bold').fontSize(7.5).fillColor(TEXT_ON_P).text(nc.numero_comprobante ?? '-', BX_PAD, by, { width: BADGE_IW, align: 'center', lineBreak: false });
    by += 11;

    doc.moveTo(BX_PAD, by).lineTo(BX_PAD + BADGE_IW, by).strokeColor(lighten(PRIMARY, 0.3)).lineWidth(0.4).stroke();
    by += 5;

    if (isAutorizado && nc.fecha_autorizacion) {
      ...
      by += 8;
      doc.font('Helvetica-Bold').fontSize(6.5).fillColor(TEXT_ON_P).text(fmtDateTime(nc.fecha_autorizacion), BX_PAD, by, { width: BADGE_IW, align: 'center', lineBreak: false });
      by += 10;
    }

    const ambLabel = nc.id_ambiente === 2 ? 'PRODUCCIÓN' : 'PRUEBAS';
    ...
    if (nc.clave_acceso) {
      ...
      doc.font('Helvetica').fontSize(5.8).fillColor(MUTED).text(nc.clave_acceso, M + 8, y + 16 + BC_H + 2, { width: CW - 80, lineBreak: false });

      const ESTADO_COLOR: Record<string, string> = {
        AUTORIZADO: '#059669', BORRADOR: '#d97706', RECHAZADA: '#dc2626', ENVIADO: '#0284c7', ANULADA: '#64748b',
      };
      const ec = ESTADO_COLOR[nc.estado] ?? '#64748b';
      doc.rect(M + CW - 70, y + 16 + BC_H - 1, 62, 14).fill(ec);
      doc.font('Helvetica-Bold').fontSize(7).fillColor('#ffffff')
         .text(nc.estado, M + CW - 70, y + 16 + BC_H + 3, { width: 62, align: 'center', lineBreak: false });

      y += SECTION_H + 5;
    }

    // ── DATOS DEL COMPRADOR Y REFERENCIA ────────────────────────────────────
    const halfCW = CW / 2 - 1;
    const compradorRows: Array<[string, string, string, string]> = [
      ['Razón Social:', nc.cli_razon_social ?? '-', 'Fecha Emisión:', fmtDate(nc.fecha_emision)],
      ['RUC / CI:', nc.cli_identificacion ?? '-', 'Doc. Referencia:', nc.factura_ref_numero ?? '-'],
      ['Motivo:', nc.motivo ?? '-', 'Fecha Doc. Ref.:', fmtDate(nc.factura_ref_fecha)],
    ];
